...
def global_correlation_softmax_3d(feature0, feature1, xyzs1, xyzs2,
    ...
    # global correlation
    b, c, n0 = feature0.shape
    b, c, n1 = feature1.shape
    feature0 = feature0.permute(0, 2, 1)  # [B, N, C]
    feature1 = feature1.view(b, c, -1)  # [B, C, N]

    correlation = torch.matmul(feature0, feature1).view(b, n0, n1) / (c ** 0.5)  # [B, N, N]
    # np.savez('correlation.npz', cor = correlation[0].detach().cpu().numpy())
    ...
    correlation = correlation.view(b, n0, n1)  # [B, N, N]

    prob = F.softmax(correlation, dim=-1)  # [B, N, N]
    
    correspondence = torch.matmul(prob, grid_2).view(b, n0, 2).permute(0, 2, 1)  # [B, 3, N]

...
class IntermediateDecoder(nn.Module):
    def __init__(self, in_ch, out_ch, skip_ch) -> None:
        super().__init__()
        self.fusion = nn.Conv2d(in_ch + 16, in_ch, kernel_size=1, bias=True)
        self.flow_fusion = nn.Conv2d(4, 2, kernel_size=1, bias=True)
        self.convblock = nn.Sequential(
            convrelu(in_ch*3+4, in_ch*3), 
            ResBlock(in_ch*3, skip_ch), 
            nn.ConvTranspose2d(in_ch*3, out_ch+4, 4, 2, 1, bias=True)
        )
    def forward(self, ft_, pft0, pft1, f0, f1, flow0_in, flow1_in, pflow0, pflow1):
        f0_warp = warp(f0, flow0_in)
        f1_warp = warp(f1, flow1_in)
        f0_warp = self.fusion(torch.cat([f0_warp, pft0], 1))
        f1_warp = self.fusion(torch.cat([f1_warp, pft1], 1))
        flow0_in = self.flow_fusion(torch.cat([flow0_in, pflow0], 1))
        flow1_in = self.flow_fusion(torch.cat([flow1_in, pflow1], 1))

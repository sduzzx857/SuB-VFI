...
class MultiFlowDecoder(nn.Module):
    ...
    def forward(self, ft_, pft0, pft1, f0, f1, flow0, flow1, pflow0, pflow1):
        n = self.num_flows
        f0_warp = warp(f0, flow0)
        f1_warp = warp(f1, flow1)
        f0_warp = self.fusion(torch.cat([f0_warp, pft0], 1))
        f1_warp = self.fusion(torch.cat([f1_warp, pft1], 1))
        flow0 = self.flow_fusion(torch.cat([flow0, pflow0], 1))
        flow1 = self.flow_fusion(torch.cat([flow1, pflow1], 1))
